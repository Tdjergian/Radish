...
echo "$JSON_OUTPUT" > "server/output/AWSIData.json"
...
bashScriptController.createRedisClusterScript = (req: Request, res: Response, next: NextFunction) => {
  const { replicas, port } = req.body;
  const outputDir = path.join(__dirname, '../output/bash-scripts');
  const awsData = require('../output/AWSIData.json');
  const ips: String[] = awsData.ips;

  let bashScript: String = `#!/bin/bash
redis-cli --cluster create`;
 ...
 ips.forEach((ip: String) => {
  ...
  bashScript += `--cluster-replicas ${replicas}`

  fs.writeFile(`${outputDir}/createRedisCluster.sh`, bashScript, (err: Error) => {
    if (err) {
      console.error('Error writing bash script:', err);
      return res.status(500).send('Internal Server Error');
    }
