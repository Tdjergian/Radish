...
const { benchmark, getSystemMetrics } = require("./benchmark");
...
app.post("/createFiles", (req, res) => {
  ...
  dockerCompose.services["app"] = {
    ...
    ports: ["80:80"],
    environment: [
      "REDIS_HOST=Redis-0-0",
      `REDIS_PORT=${portNumber}`,
      `REDIS_PASSWORD=${masterAuth}`,
      "USE_REDIS=true",
    ],
  ...
  for (let i = 0; i < numShards; i++) {
    for (let j = 0; j < numReplicas; j++) {
      ...
      if (j !== 0)
        redisConfigContent += `replicaof ${masterServiceName} ${port - j}\n`;
      ...
      fs.writeFile(outputFile, redisConfigContent, (err) => {
        ...
        if (redisFilesWritten === redisFilesToWrite) {
          ...
          fs.writeFile(dockerComposeFile, yaml.dump(dockerCompose), (err) => {
            ...
            res.status(200).json({
              message: `All configuration files written successfully to path: ${outputDir}`,
            });
          });
        }
      });
...
app.post("/benchmark", async (req, res) => {
  try {
    const { clusterNodes, numRequests, parallelClients } = req.body;
    const result = await benchmark(clusterNodes, numRequests, parallelClients);
...
app.post("/systemMetrics", async (req, res) => {
  try {
    const metrics = await getSystemMetrics();
    res.status(200).josn(metrics);
  } catch (error) {
